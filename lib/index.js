...
exports.register = function (server, options, next) {
    ...
    server.ext('onPostAuth', function (request, reply) {
        ...
        if (settings.skip && settings.skip(request, reply)) {
            server.log(['plugin', 'crumb'], { message: 'crumb response: skip.' });
        ...
        if (settings.restful === false ||
            ...
            if (request.method !== 'post' ||
                !request.route.settings.plugins._crumb) {
                server.log(['plugin', 'crumb'], { message: 'crumb response: no crumb.' });
            ...
            if (!content || content instanceof Stream) {
                server.log(['plugin', 'crumb'], { message: 'crumb response 403: No content.' });

                return reply(Boom.forbidden());
            }

            if (content[request.route.settings.plugins._crumb.key] !== request.plugins.crumb) {
                server.log(['plugin', 'crumb'], { message: 'crumb response 403: crumb dont match.' });
                return reply(Boom.forbidden());
            }
            ...
            if (!header) {
                server.log(['plugin', 'crumb'], { message: 'crumb response 403: No header.' });

                return reply(Boom.forbidden());
            }

            if (header !== request.plugins.crumb) {
                server.log(['plugin', 'crumb'], { message: 'crumb response 403: crumb header dont match.' });
                return reply(Boom.forbidden());
            }
